...
export default function SellGold() {
  ...
  if (weightParam && weight === "") {
    setWeight(weightParam);
  }

  if (karatParam && karat === "24") {
    setKarat(karatParam);
  }
...
<input type="checkbox" checked={unknownWeight} onChange={(e)=>setUnknownWeight(e.target.checked)} className="mr-2"/>
...
<label className="flex items-center text-sm">
<input type="checkbox" checked={unknownKarat} onChange={(e)=>setUnknownKarat(e.target.checked)} className="mr-2"/>
...
{tierLabel&&(
<p className="text-xs text-gray-700">{tierLabel}</p>
)}
...
<input type="file" id="photo-upload" multiple onChange={handlePhotoChange} className="hidden"/>

<label htmlFor="photo-upload" className="block text-center cursor-pointer bg-gray-900 text-white py-3 rounded-lg font-semibold hover:bg-gray-800 transition">
Add Photos
</label>
...
<img src={URL.createObjectURL(file)} alt="preview" className="w-full h-24 object-cover rounded-lg border"/>
<button type="button" onClick={()=>removePhoto(index)} className="absolute top-1 right-1 bg-black/70 text-white text-xs px-2 py-0.5 rounded">
✕
</button>
</div>
))}
...
<input type="checkbox" checked={agreed} onChange={(e)=>setAgreed(e.target.checked)} className="mr-2 mt-1"/>
